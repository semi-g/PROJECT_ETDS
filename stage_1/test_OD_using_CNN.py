### This file follows a tutorial from https://gluon.mxnet.io/chapter08_computer-vision/object-detection.html
### It illustrates Object Detection using Convolutional Neural Networks
### The model used is called SSD (Single Shot MultiBox Detector)
### The model is described in the following paper: https://arxiv.org/pdf/1512.02325.pdf
...
### Create default anchor boxes

n = 40
# shape of x -> batch * channel * height * weight
x = nd.random_uniform(shape=(1, 3, n, n))

y = MultiBoxPrior(x, sizes=[.5, .25, .1], ratios=[1, 2, .5])

# generate the first anchor box for pixel at (20,20)
# with format (x_min, y_min, x_max, y_max)
boxes = y.reshape((n, n, -1, 4))
print('The first anchor box at row 21, column 21 is: ', boxes[20, 20, 0, :])


# visualize all anchor boxes generated for one pixel
...
colors = ['cyan', 'green', 'orange', 'black', 'magenta']
...
### Predict classes
# For each anchor box, we have to predict the associated class label using ConvNet's
# with kernel size 3x3 and padding size of 1x1 
# -> channel i*(num_class + 1) contains only the background
# -> channel i*(num_class + 1)+1+j contains the object from j-th class
...
def down_sample(num_filters):
    """Stack 2 Convolution, batch normalization and ReLU blocks and then
    a pooling layer to halve the feature size"""
...
print('Outputs:', 'anchors', default_anchors.shape, 
...
## Start training -> if from scratch then 30 min with GPU
...
image = cv2.imread('./data/download_1.jpg')
x = preprocess(image)
print('x', x.shape)
